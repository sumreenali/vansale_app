...
class ReturnNewInvoiceView extends Component {
  state = {
    datum: [],
    vansaleId: 0,
    from: "",
  ...
  componentDidMount() {
    ...
    cache.getItem("purchaseinvoiceReturnData", (err, value) => {
      if (value) {
        data = value
        this.setState({
          datum: value,
  ...
  render() {
    const { invoiceReturnData, vansalesData,purchaseInvoiceReturn } = this.props
    const data = purchaseInvoiceReturn ? purchaseInvoiceReturn.package.data : []
...
export default connect(mapStateToProps)(ReturnNewInvoiceView)
